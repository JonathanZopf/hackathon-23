<script lang="ts">
  import { fetchAccessToken, jobTypes, steve, fetchJobs, jobType, accessToken, isInitialized } from "$lib/stores/JobsStore";
  import { onMount } from "svelte";

  onMount(async () => {
    $accessToken = localStorage.getItem("accessToken") ?? await fetchAccessToken();
...
<div class="card max-w-sm bg-base-100 shadow-xl p-5">
  {#if $isInitialized}
